...
const TransferForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Log pending transaction first
      try {
        await supabase.from('transactions').insert({
          // Use a typical UUID format that Supabase expects
          ...
          recipients: JSON.stringify(addresses), // Store as JSON string for JSONB column
      ...
      // Log failed transaction if we haven't already logged it
      try {
        ...
        const lastTx = data && data.length > 0 ? data[0] : null;
        const isSameTimestamp = lastTx && 
          new Date(lastTx.timestamp).getTime() > (Date.now() - 30000); // Within last 30 seconds
        ...
        if (!isSameTimestamp || !isAlreadyFailed) {
          ...
          await supabase.from('transactions').insert({
            ...
            recipients: JSON.stringify(recipients.map(r => r.address)), // Store as JSON string for JSONB column
            timestamp: new Date().toISOString(),
            error: error.message
          });
