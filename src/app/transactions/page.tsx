...
export default function TransactionsPage() {
  ...
  const checkTransactionStatus = async (tx: EnhancedTransaction) => {
    ...
    try {
      ...
      if (receipt && receipt.confirmations > 0) {
        ...
        // Update the transaction via the Worker (user_id is forced from JWT)
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      // NOTE: We do NOT require `account` (the connected wallet) to be set
      // before fetching transactions. The Worker scopes by user_id (from the
      // JWT), so /history returns the same set regardless of which wallet
      // is connected. This is the bug fix.

      setIsLoading(true);

      try {
        // THE BUG FIX: call /history on the Worker. The Worker always scopes
        // by user_id (JWT sub) — NOT by wallet_address. Switching wallets no
        // longer makes prior transactions disappear.
        const { data, error } = await getTransactionHistory(session?.access_token);
        ...
        // Process JSONB data from the Worker
    ...
    fetchTransactions();
  }, [user, session, userNetworks, userContracts, refreshKey]);
  ...
  const deleteTransaction = async (txId: string) => {
    ...
    try {
      const { error } = await deleteTransactionViaWorker(txId, session?.access_token);
  ...
  const clearAllTransactions = async () => {
    ...
    try {
      // Fetch all transactions, then delete them one by one via the Worker.
      // The Worker scopes by user_id (from JWT), so only the caller's
      // transactions will be touched.
      const { data: allTxs, error: fetchErr } = await getTransactionHistory(session?.access_token);
      if (fetchErr || !allTxs) {
        toast.error('Failed to fetch transactions to clear');
        setIsDeleting(false);
        return;
      }
      ...
      for (const tx of allTxs) {
        await deleteTransactionViaWorker(tx.id, session?.access_token);
      }

      setTransactions([]);
      toast.success(`Cleared ${allTxs.length} transactions`);
    } catch (error) {
